//! Device driver for the Winbond W25q32jv
use crate::{
...
/// WinbondW25q32jv driver, generic over a QSPI programmed in indirect mode
pub struct WinbondW25q32jv<QSPI, NOW>
where
    QSPI: qspi::Indirect,
    NOW: time::Now,
{
...
impl<QSPI, NOW> ReadWrite for WinbondW25q32jv<QSPI, NOW>
where
    QSPI: qspi::Indirect,
    NOW: time::Now,
{
    ...
    fn erase(&mut self) -> nb::Result<(), Self::Error> {
        if Self::status(&mut self.qspi)?.write_in_progress {
            ...
            Self::execute_command(&mut self.qspi, Command::ChipErase, None, CommandData::None)?;
    ...
    fn write_from_blocks<I: Iterator<Item = [u8; N]>, const N: usize>(
        ...
        let mut transfer_array = [0u8; SECTOR_SIZE];
    ...
    fn label() -> &'static str { "Winbond w25q32jv (External)" }
}

impl<QSPI, NOW> WinbondW25q32jv<QSPI, NOW>
where
    QSPI: qspi::Indirect,
    NOW: time::Now,
{
    ...
    fn status(qspi: &mut QSPI) -> nb::Result<Status, Error> {
        let mut response = [0u8; 1];
        Self::execute_command(qspi, Command::ReadStatusRegister1, None, CommandData::Read(&mut response))?;
